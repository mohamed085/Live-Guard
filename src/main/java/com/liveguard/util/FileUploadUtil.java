...
public class FileUploadUtil {
    ...
    public static void saveFile(String uploadDir, String fileName,
        ...
        try (InputStream inputStream = multipartFile.getInputStream()) {
            ...
            throw new BusinessException("Could not save file: " + fileName, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
            ...
            Files.list(dirPath).forEach(file -> {
                ...
                if (!Files.isDirectory(file)) {
                    try {
                        ...
                        log.error("FileUploadUtil | cleanDir | Could not delete file: " + file);
                    }
                }
            });
        } catch (IOException ex) {
            log.error("FileUploadUtil | cleanDir | ex.getMessage() : " + ex.getMessage());
            log.error("FileUploadUtil | cleanDir | Could not list directory: " + dirPath);
        }
    }
    ...
    public static void removeDir(String dir) {
        ...
        cleanDir(dir);
        log.debug("FileUploadUtil | removeDir | cleanDir(dir) is over");
        try {
            Files.delete(Paths.get(dir));
        } catch (IOException e) {
            log.error("FileUploadUtil | removeDir | Could not remove directory: " + dir);
        }
